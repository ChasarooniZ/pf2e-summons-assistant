...
export async function summon(summonerActor, itemUuid, summonType, summonDetailsGroup) {
  ...
  for (const summonDetails of summonDetailsGroup) {
    ...
    const params = { selectedActorUuid, summonLevel, summonType, itemUuid, summonerAlliance, additionalTraits, actorModifications, summonerActor: summonerActor.toObject(), amount, itemsToAdd };
    if (game.settings.get(MODULE_ID, "gm-places-summons")) {
      socketlib.modules.get(MODULE_ID).executeAsGM("gmHandleSummon", params)
    } else {
      await handleSummonPlacementAndPost(params);
    }
  }
}

export async function handleSummonPlacementAndPost({ selectedActorUuid, summonLevel, summonType, itemUuid, summonerAlliance, additionalTraits, actorModifications, summonerActor, amount, itemsToAdd }) {
