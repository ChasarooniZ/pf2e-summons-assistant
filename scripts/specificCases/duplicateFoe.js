...
function getValueWithoutCircumstance(basePath) {
  return basePath?.modifiers
    ...
    ?.reduce(
      (sum, val) => sum - val,
      basePath?.total ?? basePath?.value ?? basePath.mod,
    );
}
  ...
  for (const part of parts) {
    if (part === "+") {
      ...
      if (diceRegex.test(part)) {
        ...
        results.push({
          dice:
            (parseInt(diceCount) || 1) * (currentSign >= 0 ? 1 : currentSign),
          die: `d${dieType}`,
          mod: 0,
        });
